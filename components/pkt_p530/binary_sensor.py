...
async def to_code(config):
    ...
    if door_opened_cfg := config.get(CONF_DOOR_OPENED):
        sens = await binary_sensor.new_binary_sensor(door_opened_cfg)
        cg.add(parent.set_door_opened_sensor(sens))

    if door_issue_cfg := config.get(CONF_DOOR_ISSUE):
        sens = await binary_sensor.new_binary_sensor(door_issue_cfg)
        cg.add(parent.set_door_issue_sensor(sens))

    if food_low_cfg := config.get(CONF_FOOD_LOW):
        sens = await binary_sensor.new_binary_sensor(food_low_cfg)
        cg.add(parent.set_food_low_sensor(sens))
